...
export default function AdminFinances({ orders }: { orders: any[] }) {
  const totalRevenue = orders.reduce((s: number, o: any) => s + (o.totalGrossAmount ? Number(o.totalGrossAmount) : 0), 0)
  ...
  return (
    <div>
      ...
      <div className="border rounded-xl p-5 bg-card mb-8 inline-block">
        ...
        <p className="text-3xl font-heading mt-1">${totalRevenue.toFixed(2)}</p>
      </div>
          ...
          <TableBody>
            {orders.length === 0 ? (
              ...
              <TableRow key={o.id}>
                ...
                <TableCell className="font-medium">${o.totalGrossAmount}</TableCell>
                <TableCell><Badge variant={statusVariant[o.status] ?? 'outline'}>{o.status}</Badge></TableCell>

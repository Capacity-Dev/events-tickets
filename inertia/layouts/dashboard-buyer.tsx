...
const navItems = [
  { label: 'Orders', href: '/dashboard/buyer/orders', icon: '📋' },
  { label: 'My Tickets', href: '/dashboard/buyer/tickets', icon: '🎫' },
]

export default function DashboardBuyerLayout({
  children,
}: {
    ...
    <div className="flex h-screen bg-background">
      <aside className="fixed inset-y-0 left-0 z-50 w-64 border-r bg-card flex flex-col lg:static lg:block max-lg:data-[open=false]:hidden" data-open={sidebarOpen || undefined}>
        <div className="flex h-14 items-center gap-2 px-6 border-b">
          <a href="/" className="font-heading text-lg text-primary no-underline">Events Tickets</a>
        </div>
      ...
      <div className="flex flex-1 flex-col min-w-0">
        <header className="flex h-14 items-center gap-4 border-b bg-card px-6">
          <button
            ...
            className="lg:hidden inline-flex items-center justify-center rounded-lg hover:bg-muted size-8"
            ...
            <DropdownMenuTrigger>
              <button type="button" className="rounded-full hover:opacity-80">
                <Avatar className="size-8">
                  <AvatarFallback className="text-xs">{children.props.user?.initials ?? 'U'}</AvatarFallback>
                </Avatar>
              </button>
            </DropdownMenuTrigger>
